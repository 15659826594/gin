...
import (
	...
	"encoding/json"
	"fmt"
	"gin/lib/gotable/cell"
	"gin/lib/gotable/exception"
	"gin/lib/gotable/util"
	"os"
	"strings"
)
...
//
// Return error _type:
//   - *exception.UnsupportedRowTypeError: It returned when the _type of the argument is not supported.
//   - *exception.RowLengthNotEqualColumnsError: It returned if the argument is _type of the Slice but the length is
//     different from the length of column.
//   - *exception.ColumnDoNotExistError: It returned if the argument is _type of the Map but contains a nonexistent
//     column as a key.
